package com.example.aos_ar_evacuation_beacon.kalman

import kotlin.math.sqrt
import kotlin.properties.Delegates

class KalmanFilter(
   R: Float, Q: Float, stateVector: Float = 1f, controlVector: Float = 0f, measureVector: Float = 1f
                  ) {
   ...
   fun filter(signal: Float, u: Float = 0f): Float {
      if (x.isNaN()) {
         ...
         // Kalman Gain
         var kalmanGain = uncertainty * this.measureVector * (1 / ((sqrt(this.measureVector) * uncertainty) + this.Q))
